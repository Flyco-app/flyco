...
### Policy evidence and participant items

`policy_acknowledgements` has RLS enabled and no application-role privileges. Fixed-search-path publication and acceptance commands derive actor, resource and policy version, then write evidence inside the lifecycle transaction; their old signatures are removed. The private item-photo bucket adds a SELECT-only booking-participant policy. Writes and deletes remain owner-only, and public request projections remain unchanged.
