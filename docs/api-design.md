...
| delivery_pending                                   | confirm delivery / sender with traveler handoff | delivered          | One-time recipient confirmation or reviewed evidence, atomic                                                    |
...
2. Begin transaction; lock trip → delivery request → booking → payment as needed in that order.
...
5. Commit; return stable DTO. Workers use leased `FOR UPDATE SKIP LOCKED` batches, deduplication and bounded retry.
