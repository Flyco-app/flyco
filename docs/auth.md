...
Status: member authentication/profile controls and the Phase 1J staff boundary are implemented. Staff commands require a live private assignment and Supabase AAL2; metadata role claims are ignored.
...
Authentication email links use server-side `token_hash` verification and fixed, type-specific application destinations. The application rejects unapproved return paths and never accepts an arbitrary provider redirect. Auth errors must not reveal account existence. Confirmation and recovery tokens are single-use; password reset revokes other sessions according to approved policy. Hosted custom SMTP remains disabled until the Resend domain is verified; local emails go only to local Mailpit. Rate limit by IP and account fingerprint before invoking Auth endpoints.
