...
## Phase 1H verification contract

Checks cover missing acknowledgements, server-selected versions, denied direct evidence writes, participant booking details, public privacy, private Storage policies, localized policy pages and Arabic RTL. Exact-head verification remains format, lint, strict types, unit/coverage, build, Playwright, database reset/lint/pgTAP, Auth/Data API/Storage/domain checks, secret scan and reachable-history scan.
